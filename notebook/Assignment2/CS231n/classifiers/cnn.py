...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7, hidden_dim=100,
        ...
        self.dtype = dtype
        self.bn_params = {} 
        # this is used to stored the runing_mean and runing_var, 
        # which will not be optimized by the SGD
        ...
        # All weights and biases should be stored in the dictionary sel.params.     #
        # Store weights and biases for the convolutional layer using the keys 'W1'  #
        # and 'b1'; use keys 'W2' and 'b2' for the weights ans biases of the hidden #
        # affine layer, ,and keys 'W3' and 'b3' for the weights and biases of the   #
        # output affine layer.         
        #############################################################################
        ...
        # F give the number of filters, C,HH,WW characterize the size of each filter
        # Input size : (N,C,H,W)
        ...
        stride_conv = 1
        P = np.int((filter_size - 1) / 2)
        Hc = np.int((H + 2 * P - filter_height) / stride_conv) + 1
        Wc = np.int((W + 2 * P - filter_width) / stride_conv) + 1
        ...
        height_pool = 2
        stride_pool = 2
        Hp = np.int((Hc - height_pool) / stride_pool) + 1
        Wp = np.int((Wc - width_pool) / stride_pool) + 1
    ...
    def loss(self, X, y=None):
        ...
        conv_param = {'stride': 1, 'pad': np.int((filter_size - 1) / 2)}
...
class FirstConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=[16, 32], filter_size=3,
        ...
        self.L = len(num_filters)  # number of conv layers
        self.M = len(hidden_dims)  # number of affine-relu blocks
        # size of the input 
        Cinput, Hinput, Winput = input_dim
        stride_conv = 1  # stride

        # initialize the weight for the conv layers
        F = [Cinput] + num_filters  # depth of each layer including the input layer
        for i in range(self.L):
            idx = i + 1
            W = weight_scale * np.random.randn(F[i+1], F[i], self.filter_size, self.filter_size)
            b = np.zeros(F[i+1])
            ...
            if self.use_batchnorm:
                ...
                self.bn_params.update({'bn_param' + str(idx): bn_param})
                self.params.update({'gamma' + str(idx): gamma,
                                    'beta' + str(idx): beta})
                
        # initialize the weights for the affine-relu layers 
        # size of the last activation 
        Hconv, Wconv = self.Size_Conv(stride_conv, self.filter_size, Hinput, Winput, self.L)
    ...
    # calculate the conv layer output dimension using Recursive method    
    def Size_Conv(self, stride_conv, filter_size, H, W, Nbconv):
        # conv
        p = np.int((filter_size - 1) / 2) # padd zeros number
        Hc = np.int((H + 2 * p - filter_size) / stride_conv) + 1 # conv layer output height
        Wc = np.int((W + 2 * p - filter_size) / stride_conv) + 1 # conv layer output width
        # max-pooling
        width_pool = 2
        height_pool = 2
        stride_pool = 2
        Hp = np.int((Hc - height_pool) / stride_pool) + 1 # max-pooling output height
        Wp = np.int((Wc - width_pool) / stride_pool) + 1  # max-pooling output width
    ...
    def loss(self, X, y=None):
        ...
        # pass conv_param to the forward pass for the convolutional payer
        ...
        #if self.use_batchnorm:
        #    for key, nn_param in self.bn_params.items():
        #        bn_param[mode] = mode
        ...
        for i in range(self.L):
            ...
            if self.use_batchnorm:
                ...
                h, cache_h = conv_norm_relu_pool_forward(h, w, b, conv_param, pool_param, gamma, beta, bn_param)
            else:
                h, cache_h = conv_relu_pool_forward(h, w, b, conv_param, pool_param)
        ...
        # Finally forward into the score 
        idx = self.L + self.M + 1
        w = self.params['W' + str(idx)]
        b = self.params['b' + str(idx)]
        h = blocks['h' + str(idx - 1)]
        ...
        # pat attention to the dimension of the dh of affine layers
        for i in range(self.L)[::-1]:
            idx = i + 1
            dh = blocks['dh' + str(idx)]
            h_cache = blocks['cache_h' + str(idx)]
            if i == max(range(self.L)[::-1]):  # if i is the last layer of the conv, the dh  should be reshaped!
                dh = dh.reshape(*blocks['h' + str(idx)].shape)
            if self.use_batchnorm:
                dh, dw, db, dgamma, dbeta = conv_norm_relu_pool_backward(dh, h_cache)
                blocks['dbeta' + str(idx)] = dbeta
                blocks['dgamma' + str(idx)] = dgamma
            else:
        ...
        # parameters b
        list_db = {key[1:]: val for key, val in blocks.items() if ((key[:2] == 'db') and (key[:3] != 'dbe')) }
