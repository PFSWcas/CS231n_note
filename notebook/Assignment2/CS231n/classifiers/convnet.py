...
class ConvNet(object):
    """
    A convolutional network with following architecture:
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,hidden_dim=100, num_classes=10,
                 weigt_scale=1e-3, reg=0.0, dtype=np.float32, use_batchnorm=False):
        """
        ...
        # and biases for the convolutional layer using the keys 'W1' ans 'b1'; use #
        # keys 'W2' and 'b2' for the weights and biases of hidden affine layer, and#
        # keys 'W3' and 'b3' for the weights and biases of the output affine layer.#
        ############################################################################
        ...
        # Output: (N,Hh)
        Hh = hidden_dim
        W2 = weigt_scale * np.random.randn(F * Hp * Wp, Hh)
        b2 = np.zeros(Hh)
        ...
        # Output: (N,C)
        C = num_classes 
        W3 = weigt_scale * np.random.randn(Hh, C)
        b3 = np.zeros(C)
    ...
    def loss(self, X, y=None):
        ...
        W1, b1 = self.params['W1'], self.params['b1']   # parameters for the convolution layer
        W2, b2 = self.params['W2'], self.params['b2']   # parameters for the hidden net
        W3, b3 = self.params['W3'], self.params['b3']   # parameters for the affine layer
        ...
        ##################################################################
        # TODO:                                                          #
        # Implement the forward pass for the three-layer convolutional   #
        # net,computing the class scores for X and storing them in the   #
        # scores variable                                                #
        ##################################################################


        # Forward into the conv layer including pooling operation
        x = X
        w = W1
        b = b1
        ...
        dx3, dW3,db3 = affine_relu_backward(dscores, cache_scores)
        dW3 += self.reg * W3
