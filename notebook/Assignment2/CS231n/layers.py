...
def conv_backward_naive(dout, cache):
    ...
    # The convolution is equivalent to Hh * Hw neurons that sharing the parameters in a filter.
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    - gamma: Scale parameters, of shape (F, )
    ...
    - bn_param: Dictionary wiht following keys:
      ...
      - running_mean: Array of shape (F, ) giving running mean of features
      - running_var: Array of shape (F, ) giving running variance of features. 
...
def spatial_batchnorm_backward(dout, cache):
    ...
    Returns a tuple of:
    - dx: gradient with respect to inputs, of shape (N, F, H, W)
    - dgamma: gradient with respect to scale parameter, of shape (F, )
    - dbeta: gradient with respect to shift parameter, of shape (F, )
    """
    dx, dgamma, dbeta = None, None, None
    ...
    gamma = gamma.reshape(1, F, 1, 1)
    beta = beta.reshape(1, F, 1, 1)
    ...
    dx = (1./Nt) * gamma * (var+eps)**(-1./2) *(Nt*dout
           -np.sum(dout, axis=(0, 2, 3)).reshape(1, F, 1, 1)
           -(x-mu) * (var+eps)**(-1.0)*np.sum(dout*(x-mu), axis=(0, 2, 3)).reshape(1, F, 1, 1))
...
def svm_loss(x, y):
    ...
    dx = np.zeros_like(x)
    dx[margins > 0] = 1
    dx[np.arange(N), y] = 1 - x.shape[1] # 1 - x.shape[1]
    dx /= N 
    return loss, dx  
