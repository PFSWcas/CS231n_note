...
class SolverCheckpoints(object):
    ...
    The Solver performs SGD using different updates rules defined in optim.py. 

    The Solver accepts both training and validation data and labels so it can periodically
    check classification accuracy on both training and validation data to watch out for 
    overfitting. 
    ...
    After the train() method returns, model.params will contain the parameters that 
    performed best on the validation set over the course of the traing. 
    
    In addition, the instance variable solver.loss_history will contain a list of all
    losses encountered during training and the instance variables solver.train_acc_history
    and solver.val_history wil be lists containing the accuracies of the model on the training
    ...
    A Solver works on a model object that must conform to the following API:
     ...
     - model.params must be a dictionary mapping string parameter names to numpy
       arrays containing parameter valus. 
     ...
     - model.loss(x, y) must be a function that computes training-time loss and
      ...
      Returns: 
      If y is not None, run a training time forward and backward pass and return a tuple 
      of:
    ...
    def __init__(self, model, data, path, **kwargs):
        ...
        Required arguments:
        - model: A mdoel object conforming to the API described above. 
    ...
    def _reset(self):
        ...
        for p in self.model.params:
            d = {k: v for k, v in self.optim_config.items()} # a dictionary
    ...
    def load_current_checkpoints(self):
        ...
        num = max([int(f.spplit('_')[1]) for f in checkpoints])
        name = 'check_' + str(num)
    ...
    def make_check_point(self):
        ...
        joblib.dump(checkpoints, os.path.join(self.path_checkpoints, name, name + '.pkl'))
    ...
    def train(self):
        ...
        """ 
        num_train = self.X_train.shape[0]
        iterations_per_epoch = np.int(max(num_train / self.batch_size, 1))
        num_iterations = self.num_epochs * iterations_per_epoch
        
        for t in range(num_iterations):
            self._step() #  Make a single gradient update
            # Maybe print training loss
            if self.verbose and t % self.print_every == 0:
                print('Iteration "%d" / "%d", loss is "%f"' % (t + 1, num_iterations, self.loss_history[-1]))
            ...
            # Check train and val accuracy on the fitst iteration, the last iteration, and
...
class Solver(object):
    ...
    def check_accuracy(self, X, y, num_samples=None, batch_size=100):
        ...
        # Compute predictions in batches
        num_batches = np.int(np.floor(N / batch_size))
        if N % batch_size != 0:
            num_batches += 1
        y_pred = []
        for i in range(num_batches):
