...
class FirstConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=[16, 32], filter_size=3,
        ...
        F = [Cinput] + num_filters  # num_filters is a list
    ...
    def Size_Conv(self, stride_conv, filter_size, H, W, Nbconv):
        p = np.int((filter_size - 1) / 2) # padd
        Hc = np.int((H + 2 * p - filter_size) / stride_conv) + 1
        Wc = np.int((W + 2 * p - filter_size) / stride_conv) + 1
        width_pool = 2
        height_pool = 2
        stride_pool = 2
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7, 
        ...
        if self.use_batchnorm:
            print('We use batchnorm in this net')
