...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # Don't forget to L2 regularization!                                  #
        #######################################################################
        ...
        reg_los = 0.5 * self.reg * np.sum(W1**2)
        reg_los += 0.5 * self.reg * np.sum(W2**2)
...
class FullyConnectedNet(object):
    ...
    {afine - [batch norm] - relu -[dropout]} x (L - 1) - affine - softmax
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        # be initialized from a normal distribution with standard deviation     #
        # equal to weight_scale and biases should be initialized to zero.       # 
        #                                                                       #
        # when using batch normalization, store scale and shift paramters for   #
        ...
        self.L = len(hidden_dims) + 1 # number of layers in the network
        self.N = input_dim
        self.C = num_classes
        ...
        # when using dropout we need to pass a dropout_param dictionary to each droup_out
        ...
        # With batch noralization we need to keep track of running means and variances, so we need to 
        # pass a special bn_param object to each batch normalization layer. 
        # You should pass self.bn_params[0] to the forward pass of the first batch 
        # normalization layer, self.bn_params[1] to the forward pass of the second 
    ...
    def loss(self, X, y=None):
        ...
        # We are gonna store everything in a dictionary hidden 
        hidden = {}
        ...
        for i in range(self.L)[::-1]:
            ...
            if idx == self.L:
                ...
                if self.use_dropout:
                    # First backpro in the dropout layer 
